...
| POST   | `/games/draft/first-player-choice` | `{"game_id", "chosen_user_id"}`                | Requires auth; `403` if you're not seated in that game. Lets the loser of a best-of-three draft match's previous game choose either seat to go first in this one -- entirely optional, never gates `startGame()`. `409` if the game isn't `quick_draft`/`winston_draft`/`grid_draft`, has already started, is game 1 of its match (nothing to base the choice on), `chosen_user_id` isn't one of the two seated users, or the calling user wasn't the previous game's loser. See "Quick Draft"/"Winston Draft"/"Grid Draft" below. |
...
**Who goes first** (`resolveFirstPlayerId()`, `chooseFirstPlayerForNextMatchGame()`) --
...
exception: the loser of the previous game may choose either seat to go
first in the next one via `POST /games/draft/first-player-choice`
(`games.first_player_choice_user_id`, migration `0041`) -- entirely
optional, never gating `startGame()` the way deck submission does. If
the loser never calls it, the default is the previous game's own winner
going first again (`previousMatchGameWinnerUserId()`), not a fresh coin
flip -- declining the choice isn't itself a second coin flip on top of
game 1's own. `draftDeckBuildingStateFor()`'s own `first_player_choice`
field (`null` for game 1) exposes `you_are_previous_loser`,
`default_user_id`, and whatever's already been `chosen_user_id` so the
