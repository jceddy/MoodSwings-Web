...
`GET /games/deck` (see "Shared deck view" below) reuses the same
`canSpectateGame()` check for a spectator who isn't seated, so "View
decklist" works for a shared-deck game's spectators too, not just its
players.
